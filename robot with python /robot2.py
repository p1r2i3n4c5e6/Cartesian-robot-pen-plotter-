...
class MainWindow(QMainWindow):
    ...
    def create_menus(self):
        ...
        # ----- Panel toggles (checkable - reopens closed docks) -----
        panels_menu = view_menu.addMenu("Panels")
        for label, dock in [
            ("Tools Panel",            self.tool_dock),
            ("Control Panel (right)",  self.right_dock),
            ("Console / Settings / Pen Rack", self.bottom_dock),
        ]:
        ...
        view_menu.addSeparator()
        reset_action = QAction("Reset Panel Layout", self)
        reset_action.triggered.connect(self.reset_panel_layout)
    ...
    def reset_panel_layout(self):
        """Re-show all docks in their default positions."""
        for dock, area in [
            (self.tool_dock,   Qt.LeftDockWidgetArea),
            (self.right_dock,  Qt.RightDockWidgetArea),
            (self.bottom_dock, Qt.BottomDockWidgetArea),
        ]:
            if dock.isFloating():
